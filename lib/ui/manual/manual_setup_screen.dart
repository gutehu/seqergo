...
class _ManualSetupScreenState extends State<ManualSetupScreen> {
  late final ManualRepository _repo;
  late final Stream<List<ObservableClassItem>> _taxonomy;
  var _ready = false;
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<ObservableClassItem>>(
        ...
        builder: (context, snapshot) {
          final classes = snapshot.data ?? const <ObservableClassItem>[];
                ...
                child: Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: FilledButton.icon(
                      onPressed: classes.any((c) => c.subclasses.isNotEmpty)
                          ? _startSession
                          : null,
  ...
  Future<void> _addClass(BuildContext context) async {
    final name = await _promptName(context, title: "Nouvelle classe d'observables");
    if (name == null || name.isEmpty) return;
    await _repo.addClass(name);
  }
  ...
  Future<void> _startSession() async {
    ...
    await Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => ManualSessionScreen(sessionId: id),
      ),
    );
  }
}

class _EmptyTaxonomy extends StatelessWidget {
  const _EmptyTaxonomy();
    ...
    return const Center(
      child: Padding(
        ...
        child: Text(
          ...
          '(comptage ou chronomètre). Toutes les sous-classes resteront '
          'visibles pendant la session.',
